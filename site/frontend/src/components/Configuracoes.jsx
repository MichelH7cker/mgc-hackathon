...
import axios from 'axios'; // Importar axios se você estiver usando 
...
export default function Configuracoes(info) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://169.150.0.165:5173/manifest', {
                message: text,
            });

            console.log('Resposta da API:', response);
        } catch (error) {
            console.error('Erro ao enviar o texto:',    );
        }
    }
